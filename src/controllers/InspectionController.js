...
const InspectionClick = (req, res) => {
  ...
  mssql.connect(config, function (err) {
    ...
    ORDER BY case a.PTNTEXAM_STAT WHEN 'I' THEN 1 WHEN 'W' THEN 2 WHEN 'N' THEN 3 WHEN 'D' THEN 4 WHEN 'F' THEN 5 END,case a.PTNTEXAM_RMCD when 'Z2016' then 1 else 2 end,a.PTNTEXAM_TGTM, b.PTNTINFO_NAME ASC`
    request.query(q, (err, recordset) => {
      if (err) {
        res.json({
          code: 'FAIL',
